...
type IntentState = "pending_invoice" | "invoice_issued" | "invoice_uncertain" | "paid" | "authorized" | "expired" | "failed";
...
export class GateRepository {
	...
	public async markInvoiceUncertain(intentId: string): Promise<GateIntent> {
		const rows = await this.sql<IntentRow[]>`update gate_intents set state = 'invoice_uncertain', updated_at = clock_timestamp() where id = ${intentId} and state = 'pending_invoice' returning *`;
		if (rows[0] === undefined) throw new Error("GATE intent is not awaiting an invoice");
		return mapIntent(rows[0]);
	}
	...
	public async markPaid(intentId: string, paidAt: Date): Promise<GateIntent | null> {
		const rows = await this.sql<IntentRow[]>`
			...
			where id = ${intentId} and state = 'invoice_issued' and invoice_expires_at > ${paidAt} returning *
		`;
	...
	public async authorizeIntent(input: { intentId: string; receiptJti: string; now: Date; createReceipt(receiptJti: string): string }): Promise<GateAuthorization> {
		return this.sql.begin(async (tx) => {
			...
			if (intent.invoice_expires_at === null || intent.invoice_expires_at.getTime() <= input.now.getTime()) throw new Error("GATE invoice is expired");

			const projects = await tx<{ monthly_free_limit: number }[]>`select monthly_free_limit from gate_projects where id = ${intent.project_id} for update`;
			...
			const receipt = input.createReceipt(input.receiptJti);
