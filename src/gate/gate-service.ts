...
export interface GateServiceRepository {
	...
	authorizeIntent(input: { intentId: string; receiptJti: string; now: Date; createReceipt(receiptJti: string): string }): Promise<GateAuthorization>;
}
...
export class GateService {
	...
	public async poll(intentId: string): Promise<GateAuthorizationResult> {
		...
		if (intent.state === "authorized" && intent.receipt !== null) return this.#existingAuthorization(intent);
		if (intent.state !== "invoice_issued" || intent.bolt11 === null || intent.paymentHash === null || intent.invoiceExpiresAt === null) return { authorized: false, state: intent.state };
	...
	async #settleAndAuthorize(intent: GateIntent): Promise<GateAuthorizationResult> {
		...
		const authorization = await this.#repository.authorizeIntent({
			...
			now: settledAt,
			createReceipt: (receiptJti) => createGateReceipt({ issuer: this.#receipt.issuer, privateKey: this.#receipt.privateKey, keyId: this.#receipt.keyId, now: settledAt, expiresAt: new Date(settledAt.getTime() + 5 * 60_000), projectId: paid.projectId, routeId: paid.routeId, method: paid.method, path: paid.path, bodyDigest: paid.bodyDigest, amountSats: paid.amountSats, paymentHash: paid.paymentHash!, jti: receiptJti }),
		});
