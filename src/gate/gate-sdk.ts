...
export type GateIntentRequest = { intentId: string; request: GateAuthorizeRequest };
export type GateProofRequest = GateIntentRequest & { preimage: string };
export type GateClient = {
	authorize(request: GateAuthorizeRequest): Promise<GateAuthorizeResult>;
	prove(request: GateProofRequest): Promise<GateAuthorizeResult>;
	poll(request: GateIntentRequest): Promise<GateAuthorizeResult>;
};
...
export function createGateClient(options: GateClientOptions): GateClient {
	...
	if (options.routeId === undefined || options.routeId.length < 1 || options.routeId.length > 128) throw new Error("GATE route id is required for receipt verification");
	const routeId = options.routeId;
	return {
		async authorize(request) {
			const { method, body, headers } = requestHeaders(request, options);
			const response = await fetchImplementation(new URL("/api/gate/intents", baseUrl), { method: "POST", headers, body: new Uint8Array(body) });
			return handleGateResponse(response, request, method, body, options, baseUrl, routeId);
		},
		async prove({ intentId, preimage, request }) {
			if (!/^[a-f0-9]{64}$/u.test(preimage)) throw new Error("GATE preimage is invalid");
			const { method, body, headers } = requestHeaders(request, options);
			headers.set("Content-Type", "application/json");
			const response = await fetchImplementation(new URL(`/api/gate/intents/${encodeURIComponent(intentId)}/prove`, baseUrl), { method: "POST", headers, body: JSON.stringify({ preimage }) });
			return handleGateResponse(response, request, method, body, options, baseUrl, routeId);
		},
		async poll({ intentId, request }) {
			const { method, body, headers } = requestHeaders(request, options);
			const response = await fetchImplementation(new URL(`/api/gate/intents/${encodeURIComponent(intentId)}`, baseUrl), { method: "GET", headers });
			return handleGateResponse(response, request, method, body, options, baseUrl, routeId);
		},
	};
}

function requestHeaders(request: GateAuthorizeRequest, options: GateClientOptions): { method: string; body: Buffer; headers: Headers } {
...
async function handleGateResponse(response: Response, request: GateAuthorizeRequest, method: string, body: Buffer, options: GateClientOptions, baseUrl: URL, routeId: string): Promise<GateAuthorizeResult> {
	...
	if (receipt === null) throw new Error("GATE receipt verification failed");
	const claims = decodeReceiptClaims(receipt);
	if (claims === null) throw new Error("GATE receipt verification failed");
	const verified = verifyGateReceipt({ receipt, publicKey: options.receiptPublicKey, issuer: baseUrl.origin, keyId: options.receiptKeyId, expected: { projectId: options.projectId, routeId, method, path: request.path, bodyDigest: createHash("sha256").update(body).digest("hex"), amountSats: claims.amount_sats, paymentHash: claims.payment_hash, jti: claims.jti }, now: new Date() });
