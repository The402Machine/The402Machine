...
describe("pending checkout session", () => {
	it("stores only the minimum pending invoice state needed to resume", () => {
		...
		expect(storage.dump()).not.toMatch(/idempotency|ownerToken|readToken|ingestToken|pingToken|delivery|resource/iu);
	});

	it("keeps only the WHISPER decryption key required to finish its pending purchase", () => {
		const storage = memoryStorage();
		const pending = {
			version: 1 as const,
		...
		savePendingCheckout(storage, pending);
		expect(loadPendingCheckout(storage, new Date("2026-08-10T04:55:00.000Z"))).toEqual(pending);
		expect(storage.dump()).not.toMatch(/ciphertext|readLimit|revealAt|idempotency/iu);
	});
	...
	it("deletes expired, malformed, and completed pending purchases", () => {
		...
		savePendingCheckout(storage, { version: 1, product: "pulse", planId: "long", orderId: "a30d7771-76e2-4cab-b31c-45348ed9e0f7", bolt11: "lnbc4002n1pending", amountSats: 4002, expiresAt: "2026-08-10T05:00:00.000Z" });
