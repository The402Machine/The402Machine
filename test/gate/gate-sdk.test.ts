...
describe("GATE framework-neutral SDK", () => {
	it("strips spoofable GATE identity headers and forwards exact request bytes", async () => {
		...
		const client = createGateClient({ gateBaseUrl: issuer, projectId: "gate_project_123", projectKey: "gate_api_secret_123", routeKey: "weather", receiptPublicKey: publicKey, receiptKeyId: keyId, fetchImplementation });
	...
	it("supports local loopback development but rejects cleartext remote control planes", () => {
		expect(() => createGateClient({ gateBaseUrl: "http://127.0.0.1:4020", projectId: "gate_project_123", projectKey: "gate_api_secret_123", routeKey: "weather", receiptPublicKey: publicKey, receiptKeyId: keyId })).not.toThrow();
		expect(() => createGateClient({ gateBaseUrl: "http://gate.example", projectId: "gate_project_123", projectKey: "gate_api_secret_123", routeKey: "weather", receiptPublicKey: publicKey, receiptKeyId: keyId })).toThrow(/HTTPS/u);
	});
	...
	it("returns the original 402 status, headers and body without inventing success", async () => {
		...
		const client = createGateClient({ gateBaseUrl: issuer, projectId: "gate_project_123", projectKey: "gate_api_secret_123", routeKey: "weather", receiptPublicKey: publicKey, receiptKeyId: keyId, fetchImplementation: vi.fn<typeof fetch>().mockResolvedValue(new Response(responseBody, { status: 402, headers: { "www-authenticate": "Payment challenge", "content-type": "application/json" } })) });
