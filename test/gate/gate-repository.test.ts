...
describe("GateRepository", () => {
	...
	it("lists a project and its active fixed routes for operator inspection", async () => {
		const project = await createProject("inspect");
		const active = await createRoute(project.id, "active");
		const inactive = await createRoute(project.id, "inactive");
		await sql`update gate_routes set active = false where id = ${inactive.id}`;

		expect((await repository.getProjectByPublicId(project.publicId))?.id).toBe(project.id);
		expect(await repository.listRoutesForProject(project.id)).toEqual([active]);
	});
	...
	it("allows only one concurrent caller to claim invoice issuance", async () => {
		const project = await createProject("invoice-claim");
		const route = await createRoute(project.id, "invoice-claim");
		const intent = await repository.createIntent({ publicId: `gate_intent_${"v".repeat(24)}`, projectId: project.id, routeId: route.id, idempotencyKey: "invoice-claim-key", method: "POST", path: "/v1/invoice-claim", bodyDigest: "a".repeat(64), amountSats: 42, lightningAddress: "invoice-claim@example.com" });
		const claims = await Promise.all(Array.from({ length: 8 }, () => repository.claimInvoiceIssuance(intent.id)));
		expect(claims.filter(Boolean)).toHaveLength(1);
		expect((await repository.getIntent(intent.id))?.state).toBe("invoice_issuing");
	});
	...
	it("attaches one invoice and never reassigns a payment hash", async () => {
		...
		await repository.claimInvoiceIssuance(first.id);
		await repository.claimInvoiceIssuance(second.id);
		...
		await expect(repository.attachInvoice(first.id, { bolt11: "lnbc42different", paymentHash: "2".repeat(64), expiresAt, verifyUrl: null })).rejects.toThrow(/not issuing/u);
	...
	it("uses 25 UTC monthly authorizations, then consumes the earliest prepaid grant", async () => {
		...
		for (let index = 0; index < 26; index += 1) {
			...
			await repository.claimInvoiceIssuance(intent.id);
			await repository.claimInvoiceIssuance(intent.id);
	...
	it("expires only unpaid intents whose invoices are past due", async () => {
		const project = await createProject("expiry");
		const route = await createRoute(project.id, "expiry");
		const expired = await repository.createIntent({ publicId: `gate_intent_${"q".repeat(24)}`, projectId: project.id, routeId: route.id, idempotencyKey: "expiry-expired", method: "POST", path: "/v1/expiry", bodyDigest: "1".repeat(64), amountSats: 42, lightningAddress: "expiry@example.com" });
		const live = await repository.createIntent({ publicId: `gate_intent_${"r".repeat(24)}`, projectId: project.id, routeId: route.id, idempotencyKey: "expiry-live", method: "POST", path: "/v1/expiry", bodyDigest: "2".repeat(64), amountSats: 42, lightningAddress: "expiry@example.com" });
		const paid = await repository.createIntent({ publicId: `gate_intent_${"s".repeat(24)}`, projectId: project.id, routeId: route.id, idempotencyKey: "expiry-paid", method: "POST", path: "/v1/expiry", bodyDigest: "3".repeat(64), amountSats: 42, lightningAddress: "expiry@example.com" });
		await repository.claimInvoiceIssuance(expired.id);
		await repository.claimInvoiceIssuance(live.id);
		await repository.claimInvoiceIssuance(paid.id);
		await repository.attachInvoice(expired.id, { bolt11: "lnbc42expired", paymentHash: "3".repeat(64), expiresAt: new Date("2026-08-11T11:59:00Z"), verifyUrl: null });
		await repository.attachInvoice(live.id, { bolt11: "lnbc42live", paymentHash: "4".repeat(64), expiresAt: new Date("2026-08-11T12:01:00Z"), verifyUrl: null });
		await repository.attachInvoice(paid.id, { bolt11: "lnbc42paid", paymentHash: "5".repeat(64), expiresAt: new Date("2026-08-11T11:59:00Z"), verifyUrl: null });
		await sql`update gate_intents set state = 'paid', paid_at = timestamp with time zone '2026-08-11 11:58:00+00' where id = ${paid.id}`;

		expect(await repository.expireDueIntents(50, new Date("2026-08-11T12:00:00Z"))).toBe(1);
		expect((await repository.getIntent(expired.id))?.state).toBe("expired");
		expect((await repository.getIntent(live.id))?.state).toBe("invoice_issued");
		expect((await repository.getIntent(paid.id))?.state).toBe("paid");
	});

	it("recovers an expired intent when cryptographic proof arrives within bounded settlement grace", async () => {
		const project = await createProject("settlement-grace");
		const route = await createRoute(project.id, "settlement-grace");
		const intent = await repository.createIntent({ publicId: `gate_intent_${"t".repeat(24)}`, projectId: project.id, routeId: route.id, idempotencyKey: "settlement-grace", method: "POST", path: "/v1/settlement-grace", bodyDigest: "6".repeat(64), amountSats: 42, lightningAddress: "grace@example.com" });
		await repository.claimInvoiceIssuance(intent.id);
		await repository.attachInvoice(intent.id, { bolt11: "lnbc42grace", paymentHash: "7".repeat(64), expiresAt: new Date("2026-08-11T12:00:00Z"), verifyUrl: null });
		await repository.expireDueIntents(50, new Date("2026-08-11T12:00:01Z"));

		const paid = await repository.markPaid(intent.id, new Date("2026-08-11T12:04:59Z"));
		expect(paid?.state).toBe("paid");
		const authorization = await repository.authorizeIntent({ intentId: intent.id, receiptJti: `receipt_${"g".repeat(35)}`, now: new Date("2026-08-11T12:05:00Z"), createReceipt: () => "signed." + "x".repeat(80) });

		expect(authorization.source).toBe("monthly_free");
		expect((await repository.getIntent(intent.id))?.state).toBe("authorized");
	});

	it("rejects settlement proof after the bounded grace window", async () => {
		const project = await createProject("settlement-late");
		const route = await createRoute(project.id, "settlement-late");
		const intent = await repository.createIntent({ publicId: `gate_intent_${"u".repeat(24)}`, projectId: project.id, routeId: route.id, idempotencyKey: "settlement-late", method: "POST", path: "/v1/settlement-late", bodyDigest: "8".repeat(64), amountSats: 42, lightningAddress: "late@example.com" });
		await repository.claimInvoiceIssuance(intent.id);
		await repository.attachInvoice(intent.id, { bolt11: "lnbc42late", paymentHash: "9".repeat(64), expiresAt: new Date("2026-08-11T12:00:00Z"), verifyUrl: null });
		await repository.expireDueIntents(50, new Date("2026-08-11T12:00:01Z"));

		expect(await repository.markPaid(intent.id, new Date("2026-08-11T12:05:01Z"))).toBeNull();
		expect((await repository.getIntent(intent.id))?.state).toBe("expired");
	});
});
