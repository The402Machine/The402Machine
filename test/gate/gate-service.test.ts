...
function testRepository() {
	...
	type AuthorizeIntentInput = { intentId: string; receiptJti: string; now: Date; createReceipt: (receiptJti: string) => string };
	const createIntent: GateServiceRepository["createIntent"] = vi.fn((input: CreateIntentInput) => { intent = { ...baseIntent, ...input }; return Promise.resolve(intent); });
	...
	const authorizeIntent: GateServiceRepository["authorizeIntent"] = vi.fn(({ receiptJti, now: authorizedAt, createReceipt }: AuthorizeIntentInput) => {
		const receipt = createReceipt(receiptJti);
	...
	const repository: GateServiceRepository = { getRouteForProject, createIntent, attachInvoice, markInvoiceUncertain, getIntent, markPaid, getAuthorization, authorizeIntent };
	return { repository, spies: { createIntent, markInvoiceUncertain, markPaid, authorizeIntent }, intent: () => intent };
}

function service(repository: GateServiceRepository, adapter = { createInvoice: vi.fn(() => Promise.resolve(invoice)), verifyInvoice: vi.fn(() => Promise.resolve({ settled: false as const })) }) {
...
describe("GateService", () => {
	it("persists an intent before requesting one Lightning Address invoice", async () => {
		...
		expect(spies.createIntent).toHaveBeenCalledBefore(adapter.createInvoice);
