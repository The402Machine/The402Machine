...
- Agent-first through native JSON HTTP 402 and HTTP Payment Authentication. L402 remains a future compatibility layer and is not claimed by the beta.
...
- Cryptographic proof submitted immediately after invoice expiry has a bounded five-minute settlement grace. Intents beyond that window remain expired.
...
- Receipts may be retried only for the exact bound request during their five-minute validity. Origins exposing non-idempotent actions must deduplicate by receipt JTI or their own request key.
...
- Project and route provisioning is a private local operator CLI that writes directly to PostgreSQL and returns capabilities once. No public or internal HTTP provisioning endpoint is shipped.
- `inspect-project` reports non-secret project and active route policy without exposing capability hashes or plaintext credentials.
...
- Receipt verification is local in the SDK against the published JWKS.
- `GET /.well-known/gate-jwks.json` publishes receipt verification keys.
...
**TDD behaviors:**
- GATE emits native JSON 402 and HTTP Payment Authentication challenges. L402 remains a future compatibility layer and is not claimed by the beta.
...
**Implemented files:**
- `src/gate/gate-sdk.ts`
- `test/gate/gate-sdk.test.ts`
- Kept as an in-repository Node 22 module while package extraction remains future work.
